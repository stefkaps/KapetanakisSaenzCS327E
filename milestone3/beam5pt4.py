...
def run(argv=None):	
	...
	pipeline_args.extend([	
      ...
      '--job_name=zillow-rentals-4bedroom' # assign descriptive name to this job, all in lower case letters
	])
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "kapetanakissaenzcs327e:zillow.Rental_Price_4Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://kapetanakissaenzcs327e1/zillow/Zip_MedianRentalPrice_4Bedroom.csv')
