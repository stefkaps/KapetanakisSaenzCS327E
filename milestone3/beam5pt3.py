...
def parse_line(line):
	...
	start_month_index = 64; # 2015-01 is on column 58
...
def run(argv=None):	
	...
	pipeline_args.extend([	
      ...
      '--job_name=zillow-rentals-3bedroom' # assign descriptive name to this job, all in lower case letters
	])
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "kapetanakissaenzcs327e:zillow.Rental_Price_3Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://kapetanakissaenzcs327e1/zillow/Zip_MedianRentalPrice_3Bedroom.csv')
